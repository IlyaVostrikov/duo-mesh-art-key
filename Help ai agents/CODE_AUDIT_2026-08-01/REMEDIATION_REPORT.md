...
**Всего:** 33 из 52 находок исправлено (63%). 14 P0 из 14 исправлено (100%).  
...
| **P1-06** | ZIP bomb и утечки частичных файлов | Трёхслойная защита: лимит сжатого архива (100MB), бюджет несжатых данных (500MB) до записи на диск, ratio guard (>100:1 → отклонён), try/finally cleanup частичных файлов | `backend/src/services/upload.service.ts` |
...
| Находок P1 исправлено | 0 | 16 из 26 (62%) |
...
2. **P1-07** (конфликт лимитов) — требует продуктового решения по единым лимитам 3D/ZIP/storage.
